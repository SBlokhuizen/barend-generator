    ...
    const step = (idx) => {
        ...
        if (settings.enable3d && DOM.canvasContainer.style.display !== "none") {
            ...
            const highlightWhenVisible = (attempt = 0) => {
                ...
                const container = DOM.grid.querySelector(
                    `.grid-item-container[data-index='${globalIndex}']`,
                );
                if (container) {
        ...
        const step = () => {
            ...
            if (is3D) {
                ...
                const findAndValidate = (attempt = 0) => {
                    ...
                    if (container) {
                        // Apply animation to the whole container
                        container.classList.add(isValid ? "bounce" : "shake");
                        setTimeout(() => container.classList.remove("bounce", "shake"), 500);

                        const cell = container.querySelector(".cell");
                        if (cell) {
                            // Apply color class to the cell
                            cell.classList.add(isValid ? "valid" : "invalid");
                        }
                        // Use the new isValid variable
                        if (settings.enableValidationHighlight && isValid) {
                            container.classList.add("show-def-btn");
                        }
                    } else {
...
// Make this globally accessible for the UI module's click listener
window.handleDefinitionClick = handleDefinitionClick;

async function handleDefinitionClick(word) {
    UI.showDefinitionModal(word);
    UI.setDefinitionLoading(true);

    // We use a free, public proxy to get around CORS.
    const proxyUrl = "https://api.allorigins.win/raw?url=";
    const targetUrl = `https://www.encyclo.nl/begrip/${encodeURIComponent(word)}`;

    try {
        const response = await fetch(proxyUrl + targetUrl);

        if (!response.ok) {
            throw new Error(`Proxy-verzoek mislukt: ${response.status}`);
        }
        ...
        // woorden.org
        const woordenOrgLink = doc.querySelector('h3 a[href*="woorden.org/woord/"]');

        if (woordenOrgLink) {
            const h3Element = woordenOrgLink.parentElement;
            if (h3Element) {
                const spanElement = h3Element.nextElementSibling;
                
                // START: Updated logic to get all text nodes
                if (spanElement && spanElement.classList.contains('flickr_li')) {
                    let currentNode = spanElement.nextSibling;
                    let fullText = "";

                    while (currentNode) {
                        // Stop if we hit the next definition's <br>, <hr>, or the next <h3>
                        if (currentNode.nodeName === 'BR' || currentNode.nodeName === 'HR' || currentNode.nodeName === 'H3') {
                            break;
                        }
                    ...
                    const cleanedText = fullText.trim().split('...')[0].trim();
                    
                    if (cleanedText.length > 10) { // Check for a non-empty definition
        ...
        if (!definition) {
            // 1. Try to get definitions from "mijnwoordenboek.nl" source first
            const mijnwoordenboekLink = doc.querySelector('h3 a[href*="mijnwoordenboek.nl/puzzelwoordenboek"]');


            if (mijnwoordenboekLink) {
                const h3Element = mijnwoordenboekLink.parentElement;
                if (h3Element) {
                    // The span is the next element sibling
                    const spanElement = h3Element.nextElementSibling;
                    if (spanElement && spanElement.classList.contains('flickr_li')) {
                        // The text node is the next sibling (nodeType 3)
                        const textNode = spanElement.nextSibling;
                        if (textNode && textNode.nodeType === 3) {
                            const fullText = textNode.textContent.trim();
                            // Check if this text contains the numbered list format
                            if (fullText.match(/^\d+\)/)) {
                                definition = fullText;
                            }
                        }
        ...
        // 2. Fallback: Try to get the <meta name="description"> tag content
        if (!definition) {
            const metaDesc = doc.querySelector('meta[name="description"]');
            if (metaDesc) {
                let text = metaDesc.getAttribute('content');
                // Clean up the text (removes "1) ..." and "...")
                const metaDefinition = text.replace(/^\d+\)\s*/, '').split('...')[0].trim();
                if (metaDefinition.length > 10) {
                    definition = metaDefinition;
                }
            }
        }

        // 3. Fallback: try to get the first definition from the body (another source)
        if (!definition) {
            // This selector was from the original function, targeting a different source
            const firstDefNode = doc.querySelector('.slider-wrap .flickr_li + br');
            if (firstDefNode && firstDefNode.nextSibling && firstDefNode.nextSibling.nodeType === 3) { // 3 = Text Node
                const fallbackDef = firstDefNode.nextSibling.textContent.trim().split('...')[0].trim();
                if (fallbackDef.length > 10) {
        ...
        if (definition) {
            // Format the mijnwoordenboek list for better readability
            const formattedDefinition = definition.replace(/\s(\d+)\)/g, '<br>$1)');
            UI.displayDefinition(`<p>${formattedDefinition}</p>`);
        } else {
            // Check if Encyclo gave a "not found" message
            const noResult = doc.querySelector('h3'); // Encyclo uses <h3> for "niet gevonden"
            if (noResult && noResult.textContent.toLowerCase().includes("niet gevonden")) {
                UI.displayDefinition(`<p>Er zijn geen definities gevonden voor "<b>${word}</b>".</p>`);
            } else {
                UI.displayDefinition(`<p>Kon de definitie niet automatisch uit de pagina halen.</p>`);
            }
        }

    } catch (error) {
        console.error("Fout bij ophalen definitie:", error);
        // Show a user-friendly error with a link
        const errorHtml = `
      <p>De betekenis van "<b>${word}</b>" kon niet worden opgehaald.</p>
      <p style="color: var(--muted); margin-top: 16px;">
        Dit kan een netwerkfout zijn of de proxy-service is tijdelijk offline.
      </p>
      <p style="margin-top: 16px;">
        <a href="${targetUrl}" target="_blank" rel="noopener noreferrer" class="btn primary" style="padding: 8px 12px; font-size: 14px; text-decoration: none; margin-top: 8px;">
          Open Encyclo.nl voor "${word}"
        </a>
      </p>
    `;
        UI.displayDefinition(errorHtml);
    }

    UI.setDefinitionLoading(false);
}
async function main() {
    ...
    DOM.closeDefinitionModalBtn.addEventListener("click", () => {
        DOM.definitionModal.close();
    });
